...
flowchart TD
    ...
    subgraph Walk Forward Training Loop (4-Year Train / 1-Year Out-Of-Sample)
        ...
        subgraph Machine Learning Ensemble Suite (ai_engine/ensemble.py)
...
### 1. Feature Matrix Builder (`research_engine/feature_matrix.py`)
...
### 2. Future Labeler & Triple Barrier Target Generator (`research_engine/labeler.py`)
...
### 3. HMM Market Regime Detector (`ai_engine/hmm_regime.py`)
...
### 4. Data Drift Detector (`ai_engine/drift.py`)
...
### 5. Multi-Target ML Ensemble Suite (`ai_engine/ensemble.py`)
...
### 6. Universal Conformal Predictor (`ai_engine/conformal.py`)
...
### 7. Probability Calibration Tracker (`ai_engine/calibration.py`)
...
### 9. Adaptive Sizer & Bounded Execution Policy (`ai_engine/adaptive_sizer.py` & `execution_policy_engine/policy.py`)
...
| :--- | :--- | :--- | :--- |
| **1** | `research_engine/feature_matrix.py` | `FeatureMatrixBuilder` | 63+ Stationary Feature Matrix |
| **2** | `research_engine/labeler.py` | `FutureLabeler` | $Y_{\text{dir}}$, $\text{MFE}_{10/50/90}$, $\text{MAE}_{10/50/90}$ |
| **3** | `ai_engine/hmm_regime.py` | `HMMRegimeDetector` | Soft Regime Probabilities ($P_{\text{Bull}}, P_{\text{Bear}}, P_{\text{Chop}}$) |
| **4** | `ai_engine/drift.py` | `DataDriftDetector` | PSI Score & Drift Flag |
| **5** | `ai_engine/ensemble.py` | `LightGBMCatBoostEnsemble` | $P(\text{Win})$, Expected Value ($\text{EV}$) |
| **6** | `ai_engine/conformal.py` | `UniversalConformalPredictor` | 90% Confidence Bounds & Uncertainty Score |
| **7** | `ai_engine/calibration.py` | `ProbabilityCalibrationTracker` | Expected Calibration Error (ECE) |
