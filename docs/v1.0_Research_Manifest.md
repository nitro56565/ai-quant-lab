...
- **Ensemble Signal Generator**: LightGBM + CatBoost Multi-Model Classifier & Regressor (`live_trading_engine/models/signal_engine.py`).
...
1. **Clock Abstraction (`live_trading_engine/clock.py`)**: `BaseClock` with `RealClock` (live UTC) and `ReplayClock` (backtest/replay).
2. **Market Data Adapter Layer (`live_trading_engine/data/`)**: Decoupled `BaseMarketDataProvider` interface with `OANDAMarketDataProvider` wrapping live tick streaming and REST candles.
...
4. **Execution Gateway Abstraction (`live_trading_engine/broker/`)**: `BaseExecutionGateway` supporting:
