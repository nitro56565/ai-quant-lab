"""
Stage 13: Probability Calibration & Threshold Optimization Laboratory.
Benchmarks Raw Ensemble Probabilities vs Platt Sigmoid Scaling vs Isotonic Calibration
and sweeps decision threshold shifts (-0.04 to +0.04) across the 8-Fold OOS Gauntlet (2018-2025 EURUSD H1).
"""
...
def fit_platt_probs(prob_tr, y_tr, prob_te):
    try:
        from sklearn.linear_model import LogisticRegression
        lr = LogisticRegression(C=1.0, solver='lbfgs')
        X_tr = prob_tr.reshape(-1, 1)
        X_te = prob_te.reshape(-1, 1)
        lr.fit(X_tr, y_tr)
        return lr.predict_proba(X_te)[:, 1]
    except Exception:
        return prob_te

def fit_isotonic_probs(prob_tr, y_tr, prob_te):
    try:
        from sklearn.isotonic import IsotonicRegression
        iso = IsotonicRegression(out_of_bounds='clip')
        iso.fit(prob_tr, y_tr)
        return iso.predict(prob_te)
    except Exception:
        return prob_te

def process_fold_calibration(yr, df_lbl, all_feat_cols):
    warnings.filterwarnings("ignore")
    fold_seed = 42
    ...
    # In-fold split for calibration (80% train, 20% calib)
    n_tr = len(df_tr)
    c_split = int(n_tr * 0.8)

    X_tr_fit = X_tr_mat[:c_split]; y_l_fit = y_l_tr[:c_split]; y_s_fit = y_s_tr[:c_split]
    X_tr_cal = X_tr_mat[c_split:]; y_l_cal = y_l_tr[c_split:]; y_s_cal = y_s_tr[c_split:]

    pl_lgb_raw = np.zeros(len(df_te)); pl_cat_raw = np.zeros(len(df_te)); pl_xgb_raw = np.zeros(len(df_te))
    ps_lgb_raw = np.zeros(len(df_te)); ps_cat_raw = np.zeros(len(df_te)); ps_xgb_raw = np.zeros(len(df_te))

    pl_platt = np.zeros(len(df_te)); ps_platt = np.zeros(len(df_te))
    pl_iso = np.zeros(len(df_te)); ps_iso = np.zeros(len(df_te))

    for s in range(9):
        mask_tr = (state_tr == s); mask_te = (state_te == s)
        if not np.any(mask_te): continue
        if np.sum(mask_tr) >= 30:
            # Fit Full Models for Raw Baseline
            ...
            p_lgb_t = ml_lgb.predict_proba(X_te_mat[mask_te])[:, 1]
            p_cat_t = ml_cat.predict_proba(X_te_mat[mask_te])[:, 1]
            p_xgb_t = ml_xgb.predict_proba(X_te_mat[mask_te])[:, 1]

            p_lgb_st = ms_lgb.predict_proba(X_te_mat[mask_te])[:, 1]
            p_cat_st = ms_cat.predict_proba(X_te_mat[mask_te])[:, 1]
            p_xgb_st = ms_xgb.predict_proba(X_te_mat[mask_te])[:, 1]

            pl_lgb_raw[mask_te] = p_lgb_t; pl_cat_raw[mask_te] = p_cat_t; pl_xgb_raw[mask_te] = p_xgb_t
            ps_lgb_raw[mask_te] = p_lgb_st; ps_cat_raw[mask_te] = p_cat_st; ps_xgb_raw[mask_te] = p_xgb_st

            # Fit Calibration on split
            mask_fit = mask_tr[:c_split]; mask_cal = mask_tr[c_split:]
            if np.sum(mask_fit) >= 20 and np.sum(mask_cal) >= 10:
                ml_lgb_f = LGBMClassifier(n_estimators=100, max_depth=5, learning_rate=0.03, random_state=fold_seed, n_jobs=1, verbose=-1).fit(X_tr_fit[mask_fit], y_l_fit[mask_fit])
                ms_lgb_f = LGBMClassifier(n_estimators=100, max_depth=5, learning_rate=0.03, random_state=fold_seed, n_jobs=1, verbose=-1).fit(X_tr_fit[mask_fit], y_s_fit[mask_fit])

                cal_l_tr = ml_lgb_f.predict_proba(X_tr_cal[mask_cal])[:, 1]
                cal_s_tr = ms_lgb_f.predict_proba(X_tr_cal[mask_cal])[:, 1]

                cal_l_te = ml_lgb_f.predict_proba(X_te_mat[mask_te])[:, 1]
                cal_s_te = ms_lgb_f.predict_proba(X_te_mat[mask_te])[:, 1]

                pl_platt[mask_te] = fit_platt_probs(cal_l_tr, y_l_cal[mask_cal], cal_l_te)
                ps_platt[mask_te] = fit_platt_probs(cal_s_tr, y_s_cal[mask_cal], cal_s_te)

                pl_iso[mask_te] = fit_isotonic_probs(cal_l_tr, y_l_cal[mask_cal], cal_l_te)
                ps_iso[mask_te] = fit_isotonic_probs(cal_s_tr, y_s_cal[mask_cal], cal_s_te)
            else:
                pl_platt[mask_te] = (p_lgb_t + p_cat_t + p_xgb_t) / 3.0
                ps_platt[mask_te] = (p_lgb_st + p_cat_st + p_xgb_st) / 3.0
                pl_iso[mask_te] = pl_platt[mask_te]
                ps_iso[mask_te] = ps_platt[mask_te]

        else:
            pl_lgb_raw[mask_te] = 0.30; pl_cat_raw[mask_te] = 0.30; pl_xgb_raw[mask_te] = 0.30
            ps_lgb_raw[mask_te] = 0.30; ps_cat_raw[mask_te] = 0.30; ps_xgb_raw[mask_te] = 0.30
            pl_platt[mask_te] = 0.30; ps_platt[mask_te] = 0.30
            pl_iso[mask_te] = 0.30; ps_iso[mask_te] = 0.30

    p_stack_l = (pl_lgb_raw + pl_cat_raw + pl_xgb_raw) / 3.0
    p_stack_s = (ps_lgb_raw + ps_cat_raw + ps_xgb_raw) / 3.0

    return df_te.index, p_stack_l, p_stack_s, pl_platt, ps_platt, pl_iso, ps_iso, hmm_te

def run_stage13_probability_calibration():
    safe_cores = max(1, (os.cpu_count() or 4) - 2)
    print("=================================================================================", flush=True)
    print("  🚀 STAGE 13: PROBABILITY CALIBRATION & THRESHOLD OPTIMIZATION LABORATORY", flush=True)
    print("=================================================================================", flush=True)
    print(f"  • Multi-Core Accelerator: Safe Parallelization across {safe_cores} CPU Cores")
    print("  • Benchmarking: Raw Ensemble vs Platt Scaling vs Isotonic vs Decision Threshold Sweeps\n", flush=True)
    ...
    print("Executing Calibration Fold Predictions across 8 Years...", flush=True)
    results_folds = Parallel(n_jobs=safe_cores)(
        delayed(process_fold_calibration)(yr, df_lbl, all_feat_cols) for yr in years_oos
    )

    p_raw_l = np.zeros(total_h1_bars); p_raw_s = np.zeros(total_h1_bars)
    p_platt_l = np.zeros(total_h1_bars); p_platt_s = np.zeros(total_h1_bars)
    p_iso_l = np.zeros(total_h1_bars); p_iso_s = np.zeros(total_h1_bars)
    hmm_oos = np.zeros(total_h1_bars)

    for te_indices, prl, prs, ppl, pps, pil, pis, hmm_fold in results_folds:
        fold_eval_indices = [df_eval.index.get_loc(idx) for idx in te_indices if idx in df_eval.index]
        p_raw_l[fold_eval_indices] = prl; p_raw_s[fold_eval_indices] = prs
        p_platt_l[fold_eval_indices] = ppl; p_platt_s[fold_eval_indices] = pps
        p_iso_l[fold_eval_indices] = pil; p_iso_s[fold_eval_indices] = pis
    ...
    base_thresh = np.where(hmm_oos == 1.0, 0.42, 0.36)

    calib_experiments = {
        "Control Champion: Raw Ensemble Probabilities Baseline": (p_raw_l, p_raw_s, 0.0),
        "Platt Sigmoid Calibration": (p_platt_l, p_platt_s, 0.0),
        "Isotonic Regression Calibration": (p_iso_l, p_iso_s, 0.0),
        "Shifted Threshold -0.04 (Aggressive $P \ge 0.38/0.32$)": (p_raw_l, p_raw_s, -0.04),
        "Shifted Threshold +0.02 (Conservative $P \ge 0.44/0.38$)": (p_raw_l, p_raw_s, +0.02),
        "Shifted Threshold +0.04 (Ultra-Pure $P \ge 0.46/0.40$)": (p_raw_l, p_raw_s, +0.04),
    }

    def run_calib_sim(p_l, p_s, thresh_shift):
        req_p_arr = base_thresh + thresh_shift
        ...
        return {"trades": len(closed), "ret_pct": ret_pct, "sharpe": sharpe, "max_dd": max_dd, "pf": pf}

    calib_results = {}
    base_m = None

    for name, (pl, ps, shift) in calib_experiments.items():
        t_start_exp = time.time()
        print(f"▶ Evaluating Probability Calibration / Threshold: {name}...", flush=True)
        m = run_calib_sim(pl, ps, shift)
        calib_results[name] = m
    ...
    print("=========================================================================================================================================", flush=True)
    print(f"  🏆 STAGE 13 PROBABILITY CALIBRATION & THRESHOLD OPTIMIZATION SCORECARD (TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Probability Calibration & Threshold Configuration':<56} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Delta vs Control Champion':<22}", flush=True)
    ...
    for name, m in calib_results.items():
        ...
        print(f"{name:<56} | {m['trades']:<7} | +{m['ret_pct']:<13.2f}% | {m['sharpe']:<8.2f} | {m['max_dd']:<7.2f}% | {m['pf']:<6.2f} | {diff_str:<22}", flush=True)

    print("=========================================================================================================================================", flush=True)

if __name__ == "__main__":
    run_stage13_probability_calibration()
