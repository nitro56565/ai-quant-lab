"""
Stage 5: Hyper-parameter Stability & Sensitivity Analysis Laboratory.
Evaluates tree depth (3, 5, 7), learning rates (0.01, 0.03, 0.10), estimator counts (50, 100, 200),
and regime probability hurdles across the 8-Fold Walk-Forward OOS Gauntlet (2018-2025 EURUSD H1)
using the active Champion Triple Stacking Ensemble.
"""
...
def run_stage5_hyperparameter_stability():
    print("=================================================================================", flush=True)
    print("  🔬 STAGE 5: HYPER-PARAMETER STABILITY & SENSITIVITY LABORATORY", flush=True)
    print("=================================================================================", flush=True)
    print("  • Period: 2018-2025 EURUSD H1 (8-Fold Expanding Walk-Forward OOS Gauntlet)", flush=True)
    print("  • Evaluating Sensitivity: Tree Depth (3,5,7), Learning Rate (0.01,0.03,0.10), Estimators (50,100,200)\n", flush=True)
    ...
    hp_configs = {
        "Control Champion: depth=5, lr=0.03, n_est=100": {"depth": 5, "lr": 0.03, "n_est": 100, "hurdle_offset": 0.0},
        "Tree Depth = 3 (Shallow Trees)": {"depth": 3, "lr": 0.03, "n_est": 100, "hurdle_offset": 0.0},
        "Tree Depth = 7 (Deep Trees)": {"depth": 7, "lr": 0.03, "n_est": 100, "hurdle_offset": 0.0},
        "Learning Rate = 0.01 (Slow Learning)": {"depth": 5, "lr": 0.01, "n_est": 100, "hurdle_offset": 0.0},
        "Learning Rate = 0.10 (Fast Learning)": {"depth": 5, "lr": 0.10, "n_est": 100, "hurdle_offset": 0.0},
        "Estimators = 50 (Under-fitting)": {"depth": 5, "lr": 0.03, "n_est": 50, "hurdle_offset": 0.0},
        "Estimators = 200 (Over-fitting)": {"depth": 5, "lr": 0.03, "n_est": 200, "hurdle_offset": 0.0},
        "Lower Hurdle Threshold (-0.04 Shift)": {"depth": 5, "lr": 0.03, "n_est": 100, "hurdle_offset": -0.04},
        "Higher Hurdle Threshold (+0.04 Shift)": {"depth": 5, "lr": 0.03, "n_est": 100, "hurdle_offset": +0.04},
    }

    def run_hp_experiment(cfg):
        depth = cfg["depth"]
        lr = cfg["lr"]
        n_est = cfg["n_est"]
        h_offset = cfg["hurdle_offset"]
        ...
        for yr in years_oos:
            ...
            for s in range(9):
                ...
                if np.sum(mask_tr) >= 30:
                    ml_lgb = LGBMClassifier(n_estimators=n_est, max_depth=depth, learning_rate=lr, random_state=42, n_jobs=-1, verbose=-1).fit(X_tr_mat[mask_tr], y_l_tr[mask_tr])
                    ml_cat = CatBoostClassifier(iterations=n_est, depth=depth, learning_rate=lr, random_seed=42, thread_count=-1, verbose=False).fit(X_tr_mat[mask_tr], y_l_tr[mask_tr])
                    ml_xgb = XGBClassifier(n_estimators=n_est, max_depth=depth, learning_rate=lr, random_state=42, n_jobs=-1, eval_metric="logloss").fit(X_tr_mat[mask_tr], y_l_tr[mask_tr])

                    ms_lgb = LGBMClassifier(n_estimators=n_est, max_depth=depth, learning_rate=lr, random_state=42, n_jobs=-1, verbose=-1).fit(X_tr_mat[mask_tr], y_s_tr[mask_tr])
                    ms_cat = CatBoostClassifier(iterations=n_est, depth=depth, learning_rate=lr, random_seed=42, thread_count=-1, verbose=False).fit(X_tr_mat[mask_tr], y_s_tr[mask_tr])
                    ms_xgb = XGBClassifier(n_estimators=n_est, max_depth=depth, learning_rate=lr, random_state=42, n_jobs=-1, eval_metric="logloss").fit(X_tr_mat[mask_tr], y_s_tr[mask_tr])
        ...
        req_p_arr = np.where(hmm_oos == 1.0, 0.42 + h_offset, 0.36 + h_offset)
    ...
    for name, cfg in hp_configs.items():
        print(f"▶ Running Hyper-parameter Config: {name}...", flush=True)
        m = run_hp_experiment(cfg)
        hp_results[name] = m
    ...
    print("=========================================================================================================================================", flush=True)
    print(f"  🏆 STAGE 5 HYPER-PARAMETER STABILITY SCORECARD (TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Hyper-parameter Configuration':<52} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Delta vs Control Champion':<22}", flush=True)
    print("-" * 125, flush=True)

    base_r = base_m['ret_pct']
    base_s = base_m['sharpe']

    for name, m in hp_results.items():
        ...
        print(f"{name:<52} | {m['trades']:<7} | +{m['ret_pct']:<13.2f}% | {m['sharpe']:<8.2f} | {m['max_dd']:<7.2f}% | {m['pf']:<6.2f} | {diff_str:<22}", flush=True)

    print("=========================================================================================================================================", flush=True)

if __name__ == "__main__":
    run_stage5_hyperparameter_stability()
