"""
Stage 7: High-Speed Multi-Core Parallelized Labeling Optimization & Triple Barrier Calibration Laboratory.
Features Explicit Deterministic Worker Seed Alignment for 100% Bit-Exact Parallel Reproducibility.
Evaluates TP/SL ATR multipliers (1.5:1.0, 2.5:1.5, 3.0:1.0, 1.5:1.5) and Max Holding Times (12, 24, 48 bars)
...
def process_fold(yr, df_lbl, all_feat_cols):
    warnings.filterwarnings("ignore")
    # Explicitly set deterministic random state per fold/worker
    fold_seed = 42
...
def run_stage7_labeling_optimization():
    warnings.filterwarnings("ignore")
    safe_cores = max(1, (os.cpu_count() or 4) - 2)
    print("=================================================================================", flush=True)
    print("  🚀 STAGE 7: TRIPLE BARRIER CALIBRATION (EXPLICIT DETERMINISTIC SEEDED)", flush=True)
    print("=================================================================================", flush=True)
    print(f"  • Multi-Core Accelerator: Safe Parallelization across {safe_cores} CPU Cores")
    print("  • Explicit Deterministic HMM & Model Worker Seed Alignment (seed=42)")
    ...
    df_feat[all_feat_cols] = df_feat[all_feat_cols].bfill().ffill().fillna(0.0)
    ...
    label_configs = {
        "Control Champion: TP=2.5x, SL=1.5x, Hold=24H": {"tp": 2.5, "sl": 1.5, "hold": 24},
        "Tight Barriers: TP=1.5x, SL=1.0x, Hold=12H": {"tp": 1.5, "sl": 1.0, "hold": 12},
        "Asymmetric 3:1: TP=3.0x, SL=1.0x, Hold=24H": {"tp": 3.0, "sl": 1.0, "hold": 24},
        "Symmetric 1.5:1.5: TP=1.5x, SL=1.5x, Hold=24H": {"tp": 1.5, "sl": 1.5, "hold": 24},
        "Short Holding Horizon: TP=2.5x, SL=1.5x, Hold=12H": {"tp": 2.5, "sl": 1.5, "hold": 12},
        "Long Holding Horizon: TP=2.5x, SL=1.5x, Hold=48H": {"tp": 2.5, "sl": 1.5, "hold": 48},
    }

    def run_label_experiment(tp_mult, sl_mult, max_hold):
        tb_lab = TripleBarrierLabeler(tp_atr_mult=tp_mult, sl_atr_mult=sl_mult, max_holding_bars=max_hold)
        ...
        # Multi-Core Parallel Processing across all 8 walk-forward folds with deterministic worker seeds
        ...
        for i in range(total_h1_bars):
            ...
            if in_trade:
                ...
                elif (timestamp - entry_time).total_seconds() / 3600.0 >= max_hold: stop_out = True; exit_price = close; exit_reason = 'time_limit'
            ...
            if not in_trade and pending_order is not None:
                ...
                if (i - sig_idx) > 3: pending_order = None
                    ...
                    if filled:
                        ...
                        sl_pips = (p_atr / pip_size) * sl_mult; tp_pips = (p_atr / pip_size) * tp_mult; initial_sl_dist = sl_pips * pip_size
    ...
    for name, cfg in label_configs.items():
        ...
        print(f"▶ Running Deterministic Parallel Triple Barrier Experiment: {name}...", flush=True)
        m = run_label_experiment(cfg["tp"], cfg["sl"], cfg["hold"])
        label_results[name] = m
    ...
    print("=========================================================================================================================================", flush=True)
    print(f"  🏆 STAGE 7 TRIPLE BARRIER LABELING SCORECARD (PARALLEL SEEDED - TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Triple Barrier Labeling Configuration':<54} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Delta vs Champion Control':<22}", flush=True)
    print("-" * 125, flush=True)

    base_r = base_m['ret_pct']
    base_s = base_m['sharpe']

    for name, m in label_results.items():
...
if __name__ == "__main__":
    run_stage7_labeling_optimization()
