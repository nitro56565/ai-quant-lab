"""
Stage 4: Feature Selection & Random Noise Injection Laboratory.
Evaluates Feature Family Ablation, Gaussian Noise Injection (10%, 25%, 50%),
and Synthetic Noise Feature Addition across the 8-Fold Walk-Forward OOS Gauntlet (2018-2025 EURUSD H1)
using the newly locked Champion Triple Stacking Ensemble (LGBM 33.33% + CatBoost 33.33% + XGBoost 33.33%).
"""
...
def run_stage4_feature_selection():
    print("=================================================================================", flush=True)
    print("  🔬 STAGE 4: FEATURE SELECTION & RANDOM NOISE INJECTION LABORATORY", flush=True)
    print("=================================================================================", flush=True)
    print("  • Period: 2018-2025 EURUSD H1 (8-Fold Expanding Walk-Forward OOS Gauntlet)", flush=True)
    print("  • Champion Baseline: Triple Stacking Ensemble (LGBM 33.33% + CatBoost 33.33% + XGBoost 33.33%)\n", flush=True)
    ...
    experiments = {
        "Control: All Features (Triple Stack Champion)": {"type": "control"},
        "Ablation 1: Remove Momentum Features (RSI, MACD)": {"type": "ablation", "remove": ["feat_rsi", "feat_macd", "feat_macd_signal"]},
        "Ablation 2: Remove Trend Features (ADX)": {"type": "ablation", "remove": ["feat_adx"]},
        "Ablation 3: Remove Volatility Features (ATR Rank, BBands)": {"type": "ablation", "remove": ["feat_vol_atr_pct", "feat_bb_upper", "feat_bb_lower"]},
        "Noise Injection: +10% Gaussian Noise to Features": {"type": "noise", "level": 0.10},
        "Noise Injection: +25% Gaussian Noise to Features": {"type": "noise", "level": 0.25},
        "Noise Injection: +50% Gaussian Noise to Features": {"type": "noise", "level": 0.50},
        "Synthetic Noise: Add 5 Irrelevant Noise Columns": {"type": "synthetic_noise", "count": 5},
    }

    def run_feature_experiment(exp_cfg):
        np.random.seed(42)
        df_lbl_exp = df_lbl.copy()
        feat_cols = [c for c in all_feat_cols]

        if exp_cfg["type"] == "ablation":
            feat_cols = [c for c in feat_cols if c not in exp_cfg["remove"]]

        elif exp_cfg["type"] == "noise":
            noise_level = exp_cfg["level"]
            for col in feat_cols:
                std_val = df_lbl_exp[col].std()
                if std_val > 0:
                    df_lbl_exp[col] += np.random.normal(0, noise_level * std_val, size=len(df_lbl_exp))

        elif exp_cfg["type"] == "synthetic_noise":
            for n_idx in range(1, exp_cfg["count"] + 1):
                col_name = f"feat_synthetic_noise_{n_idx}"
                df_lbl_exp[col_name] = np.random.normal(0, 1.0, size=len(df_lbl_exp))
        ...
        for yr in years_oos:
            train_end_year = yr - 1
            train_m = (df_lbl_exp.index >= "2014-01-01") & (df_lbl_exp.index <= f"{train_end_year}-12-31")
            test_m = (df_lbl_exp.index >= f"{yr}-01-01") & (df_lbl_exp.index <= f"{yr}-12-31")

            df_tr = df_lbl_exp[train_m].dropna(subset=['label_dir_long']).copy()
            df_te = df_lbl_exp[test_m].copy()
    ...
    print("=========================================================================================================================================", flush=True)
    print(f"  🏆 STAGE 4 FEATURE SELECTION & NOISE INJECTION SCORECARD (TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Feature Selection / Noise Experiment':<54} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Delta vs Control Champion':<22}", flush=True)
    ...
    for name, m in exp_results.items():
        ...
        diff_str = f"{diff_r:+.2f}% Return (Sharpe {diff_s:+.2f})" if "Control" not in name else "CHAMPION BENCHMARK 🟢"
        print(f"{name:<54} | {m['trades']:<7} | +{m['ret_pct']:<13.2f}% | {m['sharpe']:<8.2f} | {m['max_dd']:<7.2f}% | {m['pf']:<6.2f} | {diff_str:<22}", flush=True)

    print("=========================================================================================================================================", flush=True)

if __name__ == "__main__":
    run_stage4_feature_selection()
