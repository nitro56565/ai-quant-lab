"""
Stage 8: Walk-Forward Window & Re-training Frequency Laboratory.
Benchmarks 1-Year Expanding (Control Champion) vs 2-Year Expanding vs 6-Month Semiannual vs 3-Month Quarterly Re-training
across the Out-of-Sample Gauntlet (2018-2025 EURUSD H1) using the Champion Triple Stacking Ensemble and deterministic seed alignment.
"""
...
def process_wf_step(tr_start, tr_end, te_start, te_end, df_lbl, all_feat_cols):
    warnings.filterwarnings("ignore")
    fold_seed = 42
    np.random.seed(fold_seed)

    train_m = (df_lbl.index >= tr_start) & (df_lbl.index <= tr_end)
    test_m = (df_lbl.index >= te_start) & (df_lbl.index <= te_end)

    df_tr = df_lbl[train_m].dropna(subset=['label_dir_long']).copy()
    df_te = df_lbl[test_m].copy()

    if len(df_te) == 0 or len(df_tr) < 100:
        return df_te.index, np.zeros(len(df_te)), np.zeros(len(df_te)), np.zeros(len(df_te))
...
def run_stage8_walk_forward_windows():
    safe_cores = max(1, (os.cpu_count() or 4) - 2)
    print("=================================================================================", flush=True)
    print("  🚀 STAGE 8: WALK-FORWARD WINDOW & RE-TRAINING FREQUENCY LABORATORY", flush=True)
    print("=================================================================================", flush=True)
    print(f"  • Multi-Core Accelerator: Safe Parallelization across {safe_cores} CPU Cores")
    print("  • Comparing: 1-Year Expanding (Control) vs 2-Year Expanding vs 6-Month vs 3-Month Windows\n", flush=True)
    ...
    # Define Walk-Forward Step Schedules
    def build_steps_1y_exp():
        steps = []
        for yr in range(2018, 2026):
            steps.append(("2014-01-01", f"{yr-1}-12-31", f"{yr}-01-01", f"{yr}-12-31"))
        return steps

    def build_steps_2y_exp():
        steps = []
        for yr in range(2018, 2026, 2):
            steps.append(("2014-01-01", f"{yr-1}-12-31", f"{yr}-01-01", f"{yr+1}-12-31"))
        return steps

    def build_steps_6m_rolling():
        steps = []
        drates = pd.date_range("2018-01-01", "2025-12-31", freq="6MS")
        for d in drates:
            te_start = d.strftime("%Y-%m-%d")
            te_end = (d + pd.DateOffset(months=6) - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
            tr_start = (d - pd.DateOffset(years=4)).strftime("%Y-%m-%d")
            tr_end = (d - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
            steps.append((tr_start, tr_end, te_start, te_end))
        return steps

    def build_steps_3m_rolling():
        steps = []
        drates = pd.date_range("2018-01-01", "2025-12-31", freq="3MS")
        for d in drates:
            te_start = d.strftime("%Y-%m-%d")
            te_end = (d + pd.DateOffset(months=3) - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
            tr_start = (d - pd.DateOffset(years=4)).strftime("%Y-%m-%d")
            tr_end = (d - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
            steps.append((tr_start, tr_end, te_start, te_end))
        return steps
    ...
    wf_configs = {
        "Control Champion: 1-Year Expanding Window (Annual Re-train)": build_steps_1y_exp(),
        "2-Year Expanding Window (Biannual Re-train)": build_steps_2y_exp(),
        "6-Month Rolling Window (Semiannual Re-train)": build_steps_6m_rolling(),
        "3-Month Rolling Window (Quarterly Re-train)": build_steps_3m_rolling(),
    }

    def run_wf_experiment(steps_list):
        results_folds = Parallel(n_jobs=safe_cores)(
            delayed(process_wf_step)(tr_s, tr_e, te_s, te_e, df_lbl, all_feat_cols)
            for tr_s, tr_e, te_s, te_e in steps_list
        )
        ...
        # Run Simulator using 12H exit limit
        pip_size = 0.0001
    ...
    for name, steps in wf_configs.items():
        t_start_exp = time.time()
        print(f"▶ Running Walk-Forward Window Experiment: {name} ({len(steps)} steps)...", flush=True)
        m = run_wf_experiment(steps)
        wf_results[name] = m
    ...
    print("=========================================================================================================================================", flush=True)
    print(f"  🏆 STAGE 8 WALK-FORWARD RE-TRAINING FREQUENCY SCORECARD (TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Walk-Forward Re-training Configuration':<56} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Delta vs Champion Control':<22}", flush=True)
...
if __name__ == "__main__":
    run_stage8_walk_forward_windows()
