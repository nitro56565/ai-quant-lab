...
from production_deployment.canonical_backtest.run_canonical_production_backtest import run_simulation_v3_1
...
def main():
    ...
    # Convert hmm_te_26 into a 0/1 array for the simulation req_p_arr logic
    hmm_te_26_arr = np.zeros(len(df_eval_26))
    hmm_te_26_arr[:] = hmm_te_26

    # We will redefine run_simulation_v3_1 locally to inject the 0.38/0.42 hurdle
    ...
    print("Holdout 2026 Metrics for 0.38/0.42:")
    print(f"Trades: {res['trades']}")
    print(f"Return: {res['ret_pct']}%")
    print(f"MDD: -{res['mtm_max_dd']}%")
