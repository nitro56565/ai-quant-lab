#!/usr/bin/env python3
"""
Master Production Live Paper Trading Daemon v3.2 — 24/7 Engine
...
def main():
    ...
    print("=================================================================================")
    print(f"  🚀 STARTING INSTITUTIONAL LIVE PAPER TRADING DAEMON v3.2 — EURUSD")
    ...
    db_manager = DatabaseManager("local_data_workspace/databases/institutional_ledger.db")
    event_bus = EventBus()
    ...
    if broker_mode in ["OANDA_PRACTICE", "OANDA_LIVE", "OANDA"]:
        broker = OANDAExecutionEngine(config=config, order_manager=order_manager, clock=clock)
    ...
    def handle_order_request(event: Event):
        ...
        # Weekend Blocker: Do not execute new trades on the weekend
        w = now_dt.weekday()
        h_utc = now_dt.hour
        is_weekend = (w == 5) or (w == 4 and h_utc >= 21) or (w == 6 and h_utc < 21)
        if getattr(config, 'weekend_shutdown', True) and is_weekend:
            logger.warning("🛡️ Weekend Blocker VETOED Order: New entries are paused during weekend market closure.")
            event_bus.publish(Event(EventType.RISK_VETOED, {"reason": "WEEKEND_BLOCKER", "symbol": symbol, "direction": direction}))
            return
