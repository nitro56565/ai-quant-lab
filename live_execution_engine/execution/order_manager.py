...
class OrderManager:
    ...
    def __init__(self, config, clock=None):
        self.config = config
        self.clock = clock
        self.log_dir = getattr(config, "log_dir", "local_data_workspace/databases")
    ...
    def create_limit_order(self, symbol: str, signal_type: str, signal_time: datetime,
        ...
        # Dynamic 0.75% Risk Position Sizing
        sl_dist_pips = abs(limit_price - sl) / pip_size
        risk_usd = 10000.0 * (self.config.risk_per_trade_pct / 100.0) # Base $75.00 on $10k equity
        raw_lots = risk_usd / (sl_dist_pips * 10.0) if sl_dist_pips > 0 else 0.31
        lots_rounded = max(0.01, round(raw_lots, 2))
        ...
        order = {
            ...
            "risk_pct": risk_pct,
            "lots": lots_rounded,
            "expiry_hours": getattr(self.config, 'pending_order_expiry_hours', 3),
            "atr": atr
        }
        self.order_counter += 1

        if self.config.limit_retrace_atr_mult == 0.0:
            # Native Market Execution Protocol
            order["status"] = "FILLED"
            pos = {
                "position_id": f"POS_{order['order_id']}",
                "symbol": order['symbol'],
                "type": order['signal_type'],
                "entry_time": current_time.strftime("%Y-%m-%d %H:%M:%S UTC") if 'current_time' in locals() else signal_time.strftime("%Y-%m-%d %H:%M:%S UTC"),
                "entry_dt": current_time if 'current_time' in locals() else signal_time,
                "entry_price": round(limit_price, 5),
                "stop_loss": order['stop_loss'],
                "initial_stop_loss": order['stop_loss'],
                "take_profit": order['take_profit'],
                "risk_pct": order.get('risk_pct', 0.75),
                "lots": order.get('lots', 0.31),
                "atr": order.get('atr', 0.0012),
                "trail_activated": False
            }
            self.open_positions.append(pos)
            self.db.save_open_position(pos)
            self.save_state()
            logger.info(f"⚡ MARKET ORDER EXECUTED: Position {pos['position_id']} OPENED | {order['symbol']} {order['signal_type']} @ {limit_price:.5f}")
        else:
    ...
    def update_positions_on_tick(self, current_time: datetime, ask: float, bid: float) -> list:
        ...
        for ord in self.pending_orders:
            ...
            if is_filled:
                pos = {
                    ...
                    "risk_pct": ord.get('risk_pct', 0.75),
                    "lots": ord.get('lots', 0.31),
        ...
        for pos in self.open_positions:
            ...
            is_oanda_trade = "oanda_trade_id" in pos or os.getenv("OANDA_BROKER_TYPE", "LOCAL_SIMULATOR").upper() in ["OANDA_PRACTICE", "OANDA_LIVE"]
            if not is_oanda_trade and current_time.hour == 21 and pos.get('last_swap_date') != curr_date_str:
            ...
            if close_reason:
                ...
                self.db.insert_trade({
                    ...
                    "spread": pos.get('spread', 0.12),
                    "slippage": pos.get('slippage', self.config.slippage_pips),
                    "commission": self.config.commission_per_lot,
                    "fill_delay_ms": pos.get('fill_delay_ms', self.config.latency_ms),
                    "probability": pos.get('probability', 0.42),
                    "expected_value": pos.get('expected_value', pnl_pips),
                    "confidence": pos.get('confidence', 0.65),
                    "regime": pos.get('regime', "RANGE"),
                    "atr": pos.get('atr', 0.0012),
                    "atr_percentile": pos.get('vol_rank_pct', 50.0),
    ...
    def save_state(self):
        ...
        with open(self.trades_history_file, "w") as f:
            json.dump(serializable(self.closed_trades), f, indent=2)
        ...
        if os.path.exists(self.state_file):
            try:
                ...
                for pos in data.get("open_positions", []):
                    entry_time_str = pos.get('entry_time') or datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
                    pos['entry_time'] = entry_time_str
                    pos['entry_dt'] = pd.to_datetime(entry_time_str).to_pydatetime()
                    self.open_positions.append(pos)
