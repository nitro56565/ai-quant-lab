...
class FractionalDifferentiation:
    ...
    @staticmethod
    def frac_diff_fixed_width(series: pd.Series, d: float = 0.40, weight_threshold: float = 1e-4, max_window: int = 100) -> pd.Series:
        """
        Applies Fixed-Width Window Fractional Differentiation to a pandas Series.
        Uses a fixed max_window to ensure bit-identical truncation parity.
        """
        n = len(series)
        w = FractionalDifferentiation.get_weights(d, max_window, weight_threshold)
        width = len(w)
