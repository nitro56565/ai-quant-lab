...
def main():
    print("=================================================================================", flush=True)
    print("  🔬 RESEARCH: H4 CONFLUENCE FULL OOS WALK-FORWARD BACKTEST (2018-2026)", flush=True)
    print("=================================================================================\n", flush=True)

    loader = DataLoader()
    symbol = "EURUSD"
    req_1h = DataRequest(symbol=symbol, timeframe="1h", start="2014-01-01", end="2026-08-11")
    req_4h = DataRequest(symbol=symbol, timeframe="4h", start="2014-01-01", end="2026-08-11")
    ...
    df_feat_1h = feat_builder.build(df_1h.copy())
    df_feat_4h = feat_builder.build(df_4h.copy())
    ...
    df_feat = df_feat_1h.join(df_macro, how='left')
    df_feat[df_macro.columns] = df_feat[df_macro.columns].ffill()
    ...
    cert_text = f"""
========================================================
    H4 CONFLUENCE METRICS — 8-YEAR WALK-FORWARD
========================================================
...
--------------------------------------------------------
2026 UNTOUCHED HOLDOUT (H4 CONFLUENCE)
--------------------------------------------------------
