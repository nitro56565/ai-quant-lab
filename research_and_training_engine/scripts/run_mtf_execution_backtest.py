...
def run_mtf_simulation_v3_2(df_eval, df_1m, p_l, p_s, hmm_arr, initial_cap=10000.0, max_open_pos=3):
    ...
    for i in range(total_bars):
        ...
        for pos in active_positions:
            ...
            if signals_arr[i] == opposite_sig:
                ...
                current_equity += rem_net
                closed_trades.append(pos)
                pending_orders.append({"direction": opposite_sig, "limit_price": closes_1h[i] - (0.25 * atrs_1h[i]) if opposite_sig == 'BUY' else closes_1h[i] + (0.25 * atrs_1h[i]), "signal_idx": i, "atr": atrs_1h[i]})
            else:
                remaining_positions.append(pos)
        active_positions = remaining_positions
...
def main():
    print("=================================================================================", flush=True)
    print("  🔬 RESEARCH: MTF (1H ALPHA + 1M EXECUTION) FULL OOS WALK-FORWARD (2018-2026)", flush=True)
    print("=================================================================================\n", flush=True)
    ...
    print("Loading 1M Execution Data (2018-2026)...", flush=True)
    req_1m = DataRequest(symbol=symbol, timeframe="1min", start="2018-01-01", end="2026-08-11")
    ...
    print(f"▶ Executing 8-Fold OOS Walk-Forward Model Training with {len(all_feat_cols)} features (2018-2025)...", flush=True)
    ...
    print("▶ Executing 1M High-Fidelity Simulation for 2018-2025...", flush=True)
    df_1m_oos = df_1m[(df_1m.index >= "2018-01-01") & (df_1m.index <= "2025-12-31")]
    oos_res = run_mtf_simulation_v3_2(df_eval_oos, df_1m_oos, p_stack_l_oos, p_stack_s_oos, hmm_oos, max_open_pos=3)
    ...
    print("▶ Executing 1M High-Fidelity Simulation for 2026...", flush=True)
    df_1m_26 = df_1m[(df_1m.index >= "2026-01-01") & (df_1m.index <= "2026-08-11")]
    holdout_res = run_mtf_simulation_v3_2(df_eval_26, df_1m_26, p_stack_l_26, p_stack_s_26, hmm_te_26, max_open_pos=3)

    cert_text = f"""
========================================================
    MTF 1M HIGH-FIDELITY EXECUTION BACKTEST
========================================================

Total OOS Trades:       {oos_res['trades']:,}
Net OOS Return:         +{oos_res['ret_pct']:.2f}%
CAGR:                   +{oos_res['cagr_pct']:.2f}% / yr
Sharpe Ratio:           {oos_res['sharpe']:.2f}
Profit Factor:          {oos_res['pf']:.2f}
Win Rate:               {oos_res['win_rate']:.2f}%

Mark-to-Market MDD:     -{oos_res['mtm_max_dd']:.2f}%
...
2026 Sharpe Ratio:      {holdout_res['sharpe']:.2f}
