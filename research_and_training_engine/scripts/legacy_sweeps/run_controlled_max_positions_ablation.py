"""
Rigorous Controlled Ablation Laboratory Script — AI Quant Lab v5.0.
Compares Control (max_open_positions = 1) vs Treatment (max_open_positions = 3).

Strict Controlled Protocol:
- Control uses exact certified baseline logic (reproducing 3,982 / 4,020 trades).
- Treatment changes EXACTLY ONE VARIABLE: max_open_positions = 3.
- All 22 model, regime, PAE, entry, exit, friction, risk, and fold parameters remain 100% bit-identical.
- Evaluates 2018-2025 OOS 8-Fold Walk-Forward + 2026 Untouched Holdout (Jan 1 - Aug 11, 2026).
"""
...
def run_simulation(df_eval, p_l, p_s, hmm_arr, max_open_pos=1, initial_cap=10000.0):
    ...
    for i in range(total_bars):
        ...
        # 1. Active Open Positions Exit Check
        remaining_positions = []
        for pos in active_positions:
            ...
            if stop_out:
                ...
                rem_lots = pos['active_lots']; rem_gross = rem_pips * (rem_lots * 10.0); rem_comm = 7.0 * rem_lots; rem_net = rem_gross - rem_comm
        ...
        # 3. New Signal Pending Order Creation
        ...
        hourly_equity.append(current_equity)
        concurrent_history.append(len(active_positions))
    ...
    max_concurrent = max(concurrent_history) if concurrent_history else 0
    avg_concurrent = np.mean(concurrent_history) if concurrent_history else 0.0
    exposure_pct = (np.sum(np.array(concurrent_history) > 0) / len(concurrent_history)) * 100.0
...
def main():
    print("=================================================================================", flush=True)
    print("  🏆 RIGOROUS CONTROLLED ABLATION: BASELINE (max=1) VS TREATMENT (max=3)", flush=True)
    print("=================================================================================\n", flush=True)
    ...
    print("▶ Step 1: Executing 8-Fold OOS Walk-Forward Predictions (2018-2025)...", flush=True)
    ...
    print("▶ Step 2: Running Control Simulation (max_open_positions = 1)...", flush=True)
    ctrl_oos = run_simulation(df_eval_oos, p_stack_l_oos, p_stack_s_oos, hmm_oos, max_open_pos=1)

    print("▶ Step 3: Running Treatment Simulation (max_open_positions = 3)...", flush=True)
    trtm_oos = run_simulation(df_eval_oos, p_stack_l_oos, p_stack_s_oos, hmm_oos, max_open_pos=3)

    # 2. 2026 Untouched Holdout (Jan 1 - Aug 11, 2026)
    print("▶ Step 4: Running 2026 Untouched Holdout Fold...", flush=True)
    ...
    ctrl_26 = run_simulation(df_eval_26, p_stack_l_26, p_stack_s_26, hmm_te_26, max_open_pos=1)
    trtm_26 = run_simulation(df_eval_26, p_stack_l_26, p_stack_s_26, hmm_te_26, max_open_pos=3)

    print("\n=================================================================================", flush=True)
    print("  🏆 RIGOROUS CONTROLLED ABLATION SCORECARD (EURUSD H1)", flush=True)
    print("=================================================================================", flush=True)
    print(f"{'Metric':<32} | {'Control (max_pos=1)':<24} | {'Treatment (max_pos=3)':<24}", flush=True)
    print("-" * 86, flush=True)
    print(f"{'2018–2025 OOS Trades':<32} | {ctrl_oos['trades']:<24,d} | {trtm_oos['trades']:<24,d}", flush=True)
    print(f"{'2018–2025 Net Return (%)':<32} | +{ctrl_oos['ret_pct']:<23.2f}% | +{trtm_oos['ret_pct']:<23.2f}%", flush=True)
    print(f"{'2018–2025 CAGR (%)':<32} | +{ctrl_oos['cagr_pct']:<23.2f}% | +{trtm_oos['cagr_pct']:<23.2f}%", flush=True)
    print(f"{'Sharpe Ratio':<32} | {ctrl_oos['sharpe']:<24.2f} | {trtm_oos['sharpe']:<24.2f}", flush=True)
    print(f"{'Sortino Ratio':<32} | {ctrl_oos['sortino']:<24.2f} | {trtm_oos['sortino']:<24.2f}", flush=True)
    print(f"{'Profit Factor':<32} | {ctrl_oos['pf']:<24.2f} | {trtm_oos['pf']:<24.2f}", flush=True)
    print(f"{'Win Rate (%)':<32} | {ctrl_oos['win_rate']:<23.2f}% | {trtm_oos['win_rate']:<23.2f}%", flush=True)
    print(f"{'Max Drawdown (%)':<32} | -{ctrl_oos['max_dd']:<23.2f}% | -{trtm_oos['max_dd']:<23.2f}%", flush=True)
    print(f"{'95% Drawdown (%)':<32} | -{ctrl_oos['dd_95']:<23.2f}% | -{trtm_oos['dd_95']:<23.2f}%", flush=True)
    print(f"{'99% Drawdown (%)':<32} | -{ctrl_oos['dd_99']:<23.2f}% | -{trtm_oos['dd_99']:<23.2f}%", flush=True)
    print(f"{'Worst Month (%)':<32} | {ctrl_oos['worst_month']:<23.2f}% | {trtm_oos['worst_month']:<23.2f}%", flush=True)
    print(f"{'Worst Year (%)':<32} | {ctrl_oos['worst_year']:<23.2f}% | {trtm_oos['worst_year']:<23.2f}%", flush=True)
    print(f"{'Average R per Trade':<32} | {ctrl_oos['avg_r']:<24.4f}R | {trtm_oos['avg_r']:<24.4f}R", flush=True)
    print(f"{'Portfolio Exposure (%)':<32} | {ctrl_oos['exposure_pct']:<23.2f}% | {trtm_oos['exposure_pct']:<23.2f}%", flush=True)
    print(f"{'Max Concurrent Positions':<32} | {ctrl_oos['max_concurrent']:<24d} | {trtm_oos['max_concurrent']:<24d}", flush=True)
    print(f"{'Time in Drawdown (%)':<32} | {ctrl_oos['time_in_dd_pct']:<23.2f}% | {trtm_oos['time_in_dd_pct']:<23.2f}%", flush=True)
    print("-" * 86, flush=True)
    print(f"{'2026 Holdout Net Return (%)':<32} | +{ctrl_26['ret_pct']:<23.2f}% | +{trtm_26['ret_pct']:<23.2f}%", flush=True)
    print(f"{'2026 Holdout Sharpe Ratio':<32} | {ctrl_26['sharpe']:<24.2f} | {trtm_26['sharpe']:<24.2f}", flush=True)
    print(f"{'2026 Holdout Max DD (%)':<32} | -{ctrl_26['max_dd']:<23.2f}% | -{trtm_26['max_dd']:<23.2f}%", flush=True)
    print(f"{'2026 Holdout Profit Factor':<32} | {ctrl_26['pf']:<24.2f} | {trtm_26['pf']:<24.2f}", flush=True)
    print(f"{'2026 Holdout Trades':<32} | {ctrl_26['trades']:<24,d} | {trtm_26['trades']:<24,d}", flush=True)
    print("=================================================================================\n", flush=True)
