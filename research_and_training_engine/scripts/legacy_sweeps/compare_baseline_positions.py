"""
Controlled Ablation Test on Exact Master Baseline: max_open_positions = 1 vs 3
Evaluates 8-Fold Walk-Forward OOS Gauntlet (2018-2025 EURUSD H1) + 2026 Holdout under 0.75% Risk per trade.
"""
...
def run_baseline_sim(df_eval, p_l, p_s, hmm_arr, max_open_pos=1, initial_cap=10000.0):
    ...
    for i in range(total_bars):
        ...
        # 1. Active Open Positions Evaluation
        remaining_positions = []
...
def main():
    print("=================================================================================")
    print("  🏆 MASTER BASELINE ABLATION TEST: max_open_positions = 1 vs 3 (0.75% RISK TIER)")
    print("=================================================================================\n")
    ...
    print("▶ Running Master Simulation for Baseline (max_open_positions = 1)...", flush=True)
    res1 = run_baseline_sim(df_eval_oos, p_stack_l_oos, p_stack_s_oos, hmm_oos, max_open_pos=1)

    print("▶ Running Master Simulation for Overlapping Variant (max_open_positions = 3)...", flush=True)
    res3 = run_baseline_sim(df_eval_oos, p_stack_l_oos, p_stack_s_oos, hmm_oos, max_open_pos=3)

    print("\n=================================================================================")
    print("  📊 OFFICIAL BASELINE COMPARATIVE RESULTS (2018–2025 EURUSD 8-FOLD OOS)")
    print("=================================================================================")
    print(f"{'Metric':<25} | {'Baseline (max=1)':<20} | {'Ablation (max=3)':<20}")
    print("-" * 75)
    print(f"{'Total OOS Trades':<25} | {res1['trades']:<20,d} | {res3['trades']:<20,d}")
    print(f"{'Net Return (%)':<25} | +{res1['ret_pct']:<19.2f}% | +{res3['ret_pct']:<19.2f}%")
    print(f"{'Sharpe Ratio':<25} | {res1['sharpe']:<20.2f} | {res3['sharpe']:<20.2f}")
    print(f"{'Profit Factor':<25} | {res1['pf']:<20.2f} | {res3['pf']:<20.2f}")
    print(f"{'Max Drawdown (%)':<25} | -{res1['max_dd']:<19.2f}% | -{res3['max_dd']:<19.2f}%")
