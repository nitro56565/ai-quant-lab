"""
=================================================================================
  🧪 PURE UNIQUE 432 COMBINATION GRID SEARCH SWEEP & 2026 HOLDOUT CERTIFICATION
=================================================================================
Executes a 100% pure, non-overlapping grid search across ALL 432 unique strategy 
parameter combinations (3 Ratios x 2 Depths x 2 Regimes x 2 Barriers x 2 Memory Windows 
x 3 Risk Tiers x 3 PAE Hurdles) without any preset variable overrides.
...
def process_fold_custom(yr, df_lbl, all_feat_cols, max_depth=5, regime_states=9, memory_mode='expanding', fold_seed=42):
    ...
    train_end_year = yr - 1
    train_start_year = max(2014, yr - 2) if memory_mode == 'rolling_2yr' else 2014
    ...
    for s in range(total_states):
        ...
        if np.sum(mask_tr) >= 20:
            ...
            ms_cat = CatBoostClassifier(iterations=100, depth=max_depth, learning_rate=0.03, random_seed=fold_seed, thread_count=1, verbose=False).fit(X_tr_mat[mask_tr], y_s_tr[mask_tr])
...
def evaluate_pure_unique_combination_task(comb_id, comb_params, df_eval, pred_cache):
    (ratio_name, depth_val, regime_opt, barrier_opt, memory_opt, risk_val, hurdle_tup) = comb_params
    ...
    res = run_grid_simulation_pure(
        df_eval, p_l, p_s, hmm_arr,
        risk_pct=risk_val, p_range=p_range, p_trend=p_trend,
...
def build_full_prediction_cache(df_lbl, all_feat_cols, years_oos, df_eval_oos, cpu_cores):
    cache_configs = [
...
def main():
    start_t = time.time()
    print("=================================================================================", flush=True)
    print("  🧪 PURE UNIQUE 432 COMBINATION GRID SEARCH SWEEP & 2026 HOLDOUT CERTIFICATION", flush=True)
    print("=================================================================================\n", flush=True)
    ...
    print(f"▶ Step 1: Pre-computing Full 8 Prediction Models Cache using {cpu_cores} Cores...", flush=True)

    pred_cache_oos = build_full_prediction_cache(df_lbl, all_feat_cols, years_oos, df_eval_oos, cpu_cores)

    # 432 Pure Unique Grid Parameters (3 x 2 x 2 x 2 x 2 x 3 x 3 = 432)
    ...
    print(f"▶ Step 2: Executing Pure Unique 432 Combination Grid Sweep ({total_grid} Combinations)...", flush=True)

    grid_results = Parallel(n_jobs=cpu_cores)(
        delayed(evaluate_pure_unique_combination_task)(i+1, comb, df_eval_oos, pred_cache_oos)
    ...
    print("\n" + "=" * 125)
    print(f"  🏆 TOP 10 WINNING COMBINATIONS FROM PURE UNIQUE 432 OOS GRID SWEEP vs FROZEN BASELINE v1.0")
    print("=" * 125)
    ...
    res_2026_winner = run_grid_simulation_pure(
    ...
    # 3. Full 432 Sweep on 2026 Data
    print(f"▶ Running Benchmark C: Full 432 Dynamic Sweep on 2026 Untouched Data ({cpu_cores} Cores)...", flush=True)
    pred_cache_2026 = build_full_prediction_cache(df_lbl, all_feat_cols, [2026], df_eval_2026, cpu_cores)

    grid_results_2026 = Parallel(n_jobs=cpu_cores)(
        delayed(evaluate_pure_unique_combination_task)(i+1, comb, df_eval_2026, pred_cache_2026)
        for i, comb in enumerate(grid_combos)
    )
    ...
    print(f"🎉 PURE UNIQUE 432 COMBINATION GRID SEARCH & 2026 CERTIFICATION COMPLETED IN {elapsed_m:.2f} MINUTES! 🎉\n", flush=True)
