...
class StorageManager:
    """
    Manages structured storage for raw and processed market data under realtime_market_streaming/:
      • Raw Ticks:       realtime_market_streaming/downloads/raw/dukascopy/{symbol}/tick/{year}/
      • Versioned OHLC: realtime_market_streaming/output/processed/v1/{symbol}/{timeframe}/{year}.parquet
      • DataLoader OHLC: realtime_market_streaming/output/{symbol}/{timeframe}/{year}.parquet
      • Audit Metadata:  realtime_market_streaming/metadata/{symbol}/symbol_info.json & audit_metadata.json
    """
    def __init__(self, base_dir: Optional[Path] = None):
        if base_dir is None:
            base_dir = Path(__file__).resolve().parent.parent
        self.pipeline_dir = base_dir / 'realtime_market_streaming'
