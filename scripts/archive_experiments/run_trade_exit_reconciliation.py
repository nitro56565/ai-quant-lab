"""
Trade Exit Limit Reconciliation Script.
Isolates the exact difference between 12-Hour Trade Exit Limit (Stage 3 Baseline) and 24-Hour Trade Exit Limit (Stage 7 Control)
on identical model probability predictions to reconcile +523.11% / +566.07% vs +479.18%.
"""
...
def run_exit_reconciliation():
    ...
    print("=================================================================================", flush=True)
    print("  🔬 RECONCILIATION DISCOVERY: TRADE EXIT TIME LIMIT COMPARISON", flush=True)
    print("=================================================================================", flush=True)
    ...
    def run_sim_with_time_limit(max_hold_hours):
    ...
    m_12h = run_sim_with_time_limit(12.0)
    m_24h = run_sim_with_time_limit(24.0)
    m_48h = run_sim_with_time_limit(48.0)

    total_elapsed = time.time() - t0

    print("=========================================================================================================================================")
    print(f"  🏆 RECONCILIATION AUDIT: TRADE EXIT TIME LIMIT COMPARISON ON IDENTICAL SIGNALS (TOTAL TIME: {total_elapsed:.1f}s)")
    print("=========================================================================================================================================")
    print(f"{'Trade Execution Time Limit':<45} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Reconciliation Source':<25}")
    print("-" * 130)
    print(f"{'1. Trade Execution Time Limit = 12 Hours':<45} | {m_12h['trades']:<7} | +{m_12h['ret_pct']:<13.2f}% | {m_12h['sharpe']:<8.2f} | {m_12h['max_dd']:<7.2f}% | {m_12h['pf']:<6.2f} | Stage 3 Champion (+566%) 🟢")
    print(f"{'2. Trade Execution Time Limit = 24 Hours':<45} | {m_24h['trades']:<7} | +{m_24h['ret_pct']:<13.2f}% | {m_24h['sharpe']:<8.2f} | {m_24h['max_dd']:<7.2f}% | {m_24h['pf']:<6.2f} | Stage 7 Control (+479%) 🟢")
    print(f"{'3. Trade Execution Time Limit = 48 Hours':<45} | {m_48h['trades']:<7} | +{m_48h['ret_pct']:<13.2f}% | {m_48h['sharpe']:<8.2f} | {m_48h['max_dd']:<7.2f}% | {m_48h['pf']:<6.2f} | Stage 7 Long Hold (+556%) 🟢")
    print("=========================================================================================================================================")
