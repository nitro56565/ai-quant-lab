"""
3D Hyper-parameter Grid Search Protocol (27 Combinations).
Sweeps depth (3, 5, 7) x learning_rate (0.01, 0.03, 0.10) x n_estimators (50, 100, 200)
across the 8-Fold Walk-Forward OOS Gauntlet (2018-2025 EURUSD H1) using the Triple Stacking Ensemble.
"""
...
def run_3d_grid():
    print("=================================================================================", flush=True)
    print("  🔬 3D HYPER-PARAMETER GRID SEARCH LABORATORY (27 COMBINATIONS)")
    print("=================================================================================", flush=True)
    print("  • Period: 2018-2025 EURUSD H1 (8-Fold Expanding Walk-Forward OOS Gauntlet)")
    print("  • Grid: Depth (3, 5, 7) x LR (0.01, 0.03, 0.10) x N_Est (50, 100, 200)\n", flush=True)
    ...
    grid_combos = list(itertools.product(depths, lrs, n_ests))

    print(f"Executing 27 Full Walk-Forward Grid Simulations...", flush=True)
    ...
    for idx, (depth, lr, n_est) in enumerate(grid_combos):
        ...
        t_elapsed_combo = time.time() - t_start_combo
        print(f"  ✓ [{idx+1:02d}/27] Depth={depth}, LR={lr:.2f}, N_Est={n_est:03d} -> Return = {ret_pct:+.2f}%, Sharpe = {sharpe:.2f}, MDD = {max_dd:.2f}% ({t_elapsed_combo:.1f}s)", flush=True)

        grid_results.append({
            "depth": depth, "lr": lr, "n_est": n_est,
            "trades": len(closed), "ret_pct": ret_pct, "sharpe": sharpe, "max_dd": max_dd, "pf": pf
        })

    df_grid = pd.DataFrame(grid_results)
    df_grid_sorted = df_grid.sort_values(by="ret_pct", ascending=False)

    total_elapsed = time.time() - t0

    print("\n=========================================================================================================================================", flush=True)
    print(f"  🏆 MASTER 3D HYPER-PARAMETER GRID SCORECARD (27 COMBINATIONS - TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Rank':<5} | {'Depth':<6} | {'LR':<6} | {'N_Est':<6} | {'Trades':<7} | {'Net Return (%)':<15} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Verdict':<20}", flush=True)
    print("-" * 115, flush=True)

    for rank, (_, row) in enumerate(df_grid_sorted.iterrows(), 1):
        is_champ = (row['depth'] == 5 and row['lr'] == 0.03 and row['n_est'] == 100)
        verdict = "CHAMPION CONTROL 🟢" if is_champ else ("GRID OPTIMUM 👑" if rank == 1 else "PROFITABLE 🟢")
        print(f"{rank:<5} | {int(row['depth']):<6} | {row['lr']:<6.2f} | {int(row['n_est']):<6} | {int(row['trades']):<7} | {row['ret_pct']:<+14.2f}% | {row['sharpe']:<8.2f} | {row['max_dd']:<7.2f}% | {row['pf']:<6.2f} | {verdict:<20}", flush=True)
