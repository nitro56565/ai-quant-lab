"""
Test Rank 1 Grid Optimum (Depth=3, LR=0.10, N_Est=200) vs Baseline (Depth=5, LR=0.03, N_Est=100)
on 100% Untouched 2026 Forward Holdout Data (Jan 1 to Aug 11, 2026).
"""
...
def run_2026_grid_optimum():
    print("=================================================================================", flush=True)
    print("  🔒 UNTOUCHED 2026 GRID OPTIMUM TEST (DEPTH=3, LR=0.10, N_EST=200 vs CHAMPION)")
    print("=================================================================================", flush=True)
    ...
    def train_triple_stack(depth, lr, n_est):
        pl_lgb = np.zeros(total_2026_bars); pl_cat = np.zeros(total_2026_bars); pl_xgb = np.zeros(total_2026_bars)
        ps_lgb = np.zeros(total_2026_bars); ps_cat = np.zeros(total_2026_bars); ps_xgb = np.zeros(total_2026_bars)
        ...
        return p_stack_l, p_stack_s

    # Train Champion Baseline (depth=5, lr=0.03, n_est=100) and Rank 1 Grid Optimum (depth=3, lr=0.10, n_est=200)
    p_champ_l, p_champ_s = train_triple_stack(depth=5, lr=0.03, n_est=100)
    p_grid1_l, p_grid1_s = train_triple_stack(depth=3, lr=0.10, n_est=200)
    ...
    def run_sim_2026(p_l, p_s):
        ...
        if not closed: return {"trades": 0, "net_pnl": 0.0, "ret_pct": 0.0, "win_rate": 0.0, "pf": 0.0, "sharpe": 0.0, "max_dd": 0.0, "ev_usd": 0.0}
        ...
        return {"trades": len(closed), "net_pnl": net_pnl, "ret_pct": ret_pct, "pf": pf, "sharpe": sharpe, "max_dd": max_dd, "ev_usd": net_pnl / len(closed)}

    m_champ = run_sim_2026(p_champ_l, p_champ_s)
    m_grid1 = run_sim_2026(p_grid1_l, p_grid1_s)

    print("=========================================================================================================================================")
    print("  🔒 UNTOUCHED 2026 PERFORMANCE: RANK 1 GRID OPTIMUM vs CHAMPION BASELINE")
    print("=========================================================================================================================================")
    print(f"{'Hyper-parameter Configuration':<52} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Expectancy ($/Tr)':<18}")
    print("-" * 125)
    print(f"{'1. Champion Control (Depth=5, LR=0.03, N_Est=100)':<52} | {m_champ['trades']:<7} | +{m_champ['ret_pct']:<13.2f}% | {m_champ['sharpe']:<8.2f} | {m_champ['max_dd']:<7.2f}% | {m_champ['pf']:<6.2f} | ${m_champ['ev_usd']:<+17.2f}")
    print(f"{'2. Rank 1 Grid Optimum (Depth=3, LR=0.10, N_Est=200)':<52} | {m_grid1['trades']:<7} | +{m_grid1['ret_pct']:<13.2f}% | {m_grid1['sharpe']:<8.2f} | {m_grid1['max_dd']:<7.2f}% | {m_grid1['pf']:<6.2f} | ${m_grid1['ev_usd']:<+17.2f}")
    print("=========================================================================================================================================")

if __name__ == "__main__":
    run_2026_grid_optimum()
