"""
2026 Holdout Risk Allocation Verification (0.50% vs 0.75% Risk per Trade).
"""
...
def run_2026_debug():
    ...
    hmm_detector = HMMRegimeDetector(n_components=3, random_state=42)
    ...
    closes = df_te['close'].values; highs = df_te['high'].values; lows = df_te['low'].values; atrs = df_te['feat_vol_atr'].values
    ...
    m_050 = run_sim_2026(0.0050)
    m_075 = run_sim_2026(0.0075)

    print("=========================================================================================================================================")
    print("  🏆 2026 HOLDOUT RISK ALLOCATION COMPARISON (0.50% VS 0.75% RISK/TRADE)")
    print("=========================================================================================================================================")
    print(f"{'2026 Holdout Risk Allocation Tier':<52} | {'Trades':<7} | {'Net Return (%)':<15} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6}")
    print("-" * 115)
    print(f"{'0.50% Risk per Trade (1st Preference Baseline)':<52} | {m_050['trades']:<7} | +{m_050['ret_pct']:<14.2f}% | {m_050['sharpe']:<8.2f} | {m_050['max_dd']:<7.2f}% | {m_050['pf']:<6.2f}")
    print(f"{'0.75% Risk per Trade (2nd Preference High-Growth)':<52} | {m_075['trades']:<7} | +{m_075['ret_pct']:<14.2f}% | {m_075['sharpe']:<8.2f} | {m_075['max_dd']:<7.2f}% | {m_075['pf']:<6.2f}")
    print("=========================================================================================================================================")
