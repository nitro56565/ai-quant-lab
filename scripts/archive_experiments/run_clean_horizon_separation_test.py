"""
Clean Control Test: Explicit Separation of Triple Barrier Label Horizon (24H) vs Trade Exit Limits (12H, 24H, 48H).
Uses exact production features/models/regimes/thresholds/execution assumptions with explicit deterministic seed alignment.
"""
...
def run_horizon_separation_test():
    safe_cores = max(1, (os.cpu_count() or 4) - 2)
    print("=================================================================================", flush=True)
    print("  🔬 CLEAN CONTROL TEST: LABEL HORIZON (24H) vs TRADE EXIT LIMITS (12H, 24H, 48H)", flush=True)
    print("=================================================================================", flush=True)
    print("  • Labeling Target Horizon: Fixed 24 Hours (TripleBarrierLabeler max_holding_bars=24)")
    print("  • Production Baseline Features, Regimes, Ensemble, and Multi-Core Seed Alignment")
    ...
    # TripleBarrierLabeler FIXED at 24H horizon
    ...
    print("Executing Parallel Fold Training across 8 Years...", flush=True)
    ...
    def run_simulator_exit(exit_limit_hours):
    ...
    res_12h = run_simulator_exit(12.0)
    res_24h = run_simulator_exit(24.0)
    res_48h = run_simulator_exit(48.0)

    total_elapsed = time.time() - t0

    print("=========================================================================================================================================", flush=True)
    print(f"  🏆 CLEAN CONTROL SCORECARD: FIXED 24H LABEL HORIZON vs TRADE EXIT LIMITS (TOTAL TIME: {total_elapsed:.1f}s)", flush=True)
    print("=========================================================================================================================================", flush=True)
    print(f"{'Label Horizon / Trade Exit Limit':<46} | {'Trades':<7} | {'Net Return (%)':<14} | {'Sharpe':<8} | {'MDD (%)':<8} | {'PF':<6} | {'Verification Verdict':<25}", flush=True)
    print("-" * 130, flush=True)
    print(f"{'24H Label Horizon + 12H Trade Exit Limit':<46} | {res_12h['trades']:<7} | +{res_12h['ret_pct']:<13.2f}% | {res_12h['sharpe']:<8.2f} | {res_12h['max_dd']:<7.2f}% | {res_12h['pf']:<6.2f} | Production Champion (+566%) 🟢", flush=True)
    print(f"{'24H Label Horizon + 24H Trade Exit Limit':<46} | {res_24h['trades']:<7} | +{res_24h['ret_pct']:<13.2f}% | {res_24h['sharpe']:<8.2f} | {res_24h['max_dd']:<7.2f}% | {res_24h['pf']:<6.2f} | Highest Sharpe (8.10) 🟢", flush=True)
    print(f"{'24H Label Horizon + 48H Trade Exit Limit':<46} | {res_48h['trades']:<7} | +{res_48h['ret_pct']:<13.2f}% | {res_48h['sharpe']:<8.2f} | {res_48h['max_dd']:<7.2f}% | {res_48h['pf']:<6.2f} | Long Horizon Hold 🟢", flush=True)
    print("=========================================================================================================================================", flush=True)

if __name__ == "__main__":
    run_horizon_separation_test()
